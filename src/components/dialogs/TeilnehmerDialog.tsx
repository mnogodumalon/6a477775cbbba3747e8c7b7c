...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Teilnehmer';
...
export function TeilnehmerDialog({ open, onClose, onSubmit, defaultValues, recordId, anmeldungenList, enablePhotoScan = true, enablePhotoLocation = true }: TeilnehmerDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Teilnehmer['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'teilnehmer');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "anmeldung": string | null, // Display name from Anmeldungen (see <available-records>)\n  "teilnehmer_vorname": string | null, // Teilnehmer Vorname\n  "teilnehmer_nachname": string | null, // Teilnehmer Nachname\n  "teilnehmer_email": string | null, // Teilnehmer E-Mail\n  "teilnehmer_telefon": string | null, // Teilnehmer Telefon\n  "zertifikats_nr": string | null, // Zertifikats-Nr.\n  "gebuchte_stufe": LookupValue | null, // Gebuchte Stufe (select one key: "tt2" | "tt3" | "tt1") mapping: tt2=TT2, tt3=TT3, tt1=TT1\n  "gebuchtes_verfahren": LookupValue | null, // Gebuchtes Verfahren (select one key: "tt2_pb" | "tt2_pe" | "tt2_pi" | "vds") mapping: tt2_pb=TT2-PB, tt2_pe=TT2-PE, tt2_pi=TT2-PI, vds=VdS\n  "teilnehmerstatus": string | null, // Teilnehmerstatus\n  "bemerkung": string | null, // Bemerkung\n  "spz_id": string | null, // SPZ-ID\n  "spz_source": string | null, // SPZ-Quelle\n  "spz_last_sync": string | null, // YYYY-MM-DDTHH:MM\n  "spz_sync_status": string | null, // Sync-Status\n  "spz_deleted": boolean | null, // In WEBER.SPZ gelöscht\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["anmeldung"]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'teilnehmer_vorname': (
      <div key="teilnehmer_vorname" className="space-y-1.5">
        <Label htmlFor="teilnehmer_vorname">Teilnehmer Vorname</Label>
        <Input
    ...
    'teilnehmer_nachname': (
      <div key="teilnehmer_nachname" className="space-y-1.5">
        <Label htmlFor="teilnehmer_nachname">Teilnehmer Nachname</Label>
        <Input
    ...
    'teilnehmer_email': (
      <div key="teilnehmer_email" className="space-y-1.5">
        <Label htmlFor="teilnehmer_email">Teilnehmer E-Mail</Label>
        <Input
          id="teilnehmer_email"
          type="email"
          placeholder="z. B. klaus@beispiel.de"
    ...
    'teilnehmer_telefon': (
      <div key="teilnehmer_telefon" className="space-y-1.5">
        <Label htmlFor="teilnehmer_telefon">Teilnehmer Telefon</Label>
        <Input
    ...
    'zertifikats_nr': (
      <div key="zertifikats_nr" className="space-y-1.5">
        <Label htmlFor="zertifikats_nr">Zertifikats-Nr.</Label>
        <Input
          id="zertifikats_nr"
          placeholder="z. B. ZERT-123456"
          value={fields.zertifikats_nr ?? ''}
          onChange={e => setFields(f => ({ ...f, zertifikats_nr: e.target.value }))}
        />
      </div>
    ),
    'gebuchte_stufe': (
      <div key="gebuchte_stufe" className="space-y-1.5">
        <Label htmlFor="gebuchte_stufe">Gebuchte Stufe</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.gebuchte_stufe) === 'tt2'}
            onClick={() => setFields(f => ({ ...f, gebuchte_stufe: (lookupKey(f.gebuchte_stufe) === 'tt2' ? undefined : 'tt2') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.gebuchte_stufe) === 'tt2'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.gebuchte_stufe) === 'tt3'}
            onClick={() => setFields(f => ({ ...f, gebuchte_stufe: (lookupKey(f.gebuchte_stufe) === 'tt3' ? undefined : 'tt3') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.gebuchte_stufe) === 'tt3'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.gebuchte_stufe) === 'tt1'}
            onClick={() => setFields(f => ({ ...f, gebuchte_stufe: (lookupKey(f.gebuchte_stufe) === 'tt1' ? undefined : 'tt1') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.gebuchte_stufe) === 'tt1'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'gebuchtes_verfahren': (
      <div key="gebuchtes_verfahren" className="space-y-1.5">
        <Label htmlFor="gebuchtes_verfahren">Gebuchtes Verfahren</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.gebuchtes_verfahren) === 'tt2_pb'}
            onClick={() => setFields(f => ({ ...f, gebuchtes_verfahren: (lookupKey(f.gebuchtes_verfahren) === 'tt2_pb' ? undefined : 'tt2_pb') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.gebuchtes_verfahren) === 'tt2_pb'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.gebuchtes_verfahren) === 'tt2_pe'}
            onClick={() => setFields(f => ({ ...f, gebuchtes_verfahren: (lookupKey(f.gebuchtes_verfahren) === 'tt2_pe' ? undefined : 'tt2_pe') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.gebuchtes_verfahren) === 'tt2_pe'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.gebuchtes_verfahren) === 'tt2_pi'}
            onClick={() => setFields(f => ({ ...f, gebuchtes_verfahren: (lookupKey(f.gebuchtes_verfahren) === 'tt2_pi' ? undefined : 'tt2_pi') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.gebuchtes_verfahren) === 'tt2_pi'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.gebuchtes_verfahren) === 'vds'}
            onClick={() => setFields(f => ({ ...f, gebuchtes_verfahren: (lookupKey(f.gebuchtes_verfahren) === 'vds' ? undefined : 'vds') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.gebuchtes_verfahren) === 'vds'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'teilnehmerstatus': (
      <div key="teilnehmerstatus" className="space-y-1.5">
        <Label htmlFor="teilnehmerstatus">Teilnehmerstatus</Label>
        <Input
          id="teilnehmerstatus"
          placeholder="z. B. angemeldet, teilgenommen"
          value={fields.teilnehmerstatus ?? ''}
          onChange={e => setFields(f => ({ ...f, teilnehmerstatus: e.target.value }))}
        />
      </div>
    ),
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkung"
          placeholder="Notizen, Allergie, Besonderheiten..."
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.TEILNEHMER} recordId={recordId} />
              </div>
            )}
          </div>

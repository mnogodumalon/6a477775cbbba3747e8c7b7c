...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/WeberSpzUebersicht';
...
interface WeberSpzUebersichtDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: WeberSpzUebersicht['fields']) => Promise<void>;
  defaultValues?: WeberSpzUebersicht['fields'];
...
export function WeberSpzUebersichtDialog({ open, onClose, onSubmit, defaultValues, recordId, veranstaltungenList, enablePhotoScan = true, enablePhotoLocation = true }: WeberSpzUebersichtDialogProps) {
  const [fields, setFields] = useState<Partial<WeberSpzUebersicht['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<WeberSpzUebersicht['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'weber.spz_uebersicht');
      await onSubmit(clean as WeberSpzUebersicht['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="naechste_veranstaltungen" entity="Veranstaltungen">\n${JSON.stringify(veranstaltungenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "anzahl_anmeldungen": number | null, // Anzahl aktuelle Anmeldungen\n  "naechste_veranstaltungen": string | null, // Display name from Veranstaltungen (see <available-records>)\n  "gesamtteilnehmerzahl": number | null, // Gesamte Teilnehmerzahl\n  "sync_status_gesamt": string | null, // Sync-Status (gesamt)\n  "fehlerhafte_syncs": number | null, // Fehlerhafte Synchronisationen\n  "letzter_sync": string | null, // YYYY-MM-DDTHH:MM\n  "spz_id": string | null, // SPZ-ID\n  "spz_source": string | null, // SPZ-Quelle\n  "spz_last_sync": string | null, // YYYY-MM-DDTHH:MM\n  "spz_sync_status": string | null, // Sync-Status (technisch)\n  "spz_deleted": boolean | null, // In WEBER.SPZ gelöscht\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["naechste_veranstaltungen"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const naechste_veranstaltungenName = raw['naechste_veranstaltungen'] as string | null;
        if (naechste_veranstaltungenName) {
          const naechste_veranstaltungenMatch = veranstaltungenList.find(r => matchName(naechste_veranstaltungenName!, [String(r.fields.veranstaltungsnummer ?? '')]));
          if (naechste_veranstaltungenMatch) merged['naechste_veranstaltungen'] = createRecordUrl(APP_IDS.VERANSTALTUNGEN, naechste_veranstaltungenMatch.record_id);
        }
        return merged as Partial<WeberSpzUebersicht['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'WEBER.SPZ Übersicht bearbeiten' : 'WEBER.SPZ Übersicht hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'anzahl_anmeldungen': (
      <div key="anzahl_anmeldungen" className="space-y-1.5">
        <Label htmlFor="anzahl_anmeldungen">Anzahl aktuelle Anmeldungen</Label>
        <Input
          id="anzahl_anmeldungen"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'anzahl_anmeldungen')}
          placeholder="z. B. 42"
          value={fields.anzahl_anmeldungen !== undefined ? fields.anzahl_anmeldungen : (computedValues['anzahl_anmeldungen'] ?? '')}
          onChange={e => setFields(f => ({ ...f, anzahl_anmeldungen: clampNumberValue(formEnhancements, 'anzahl_anmeldungen', e.target.value) }))}
        />
      </div>
    ),
    'naechste_veranstaltungen': (
      <div key="naechste_veranstaltungen" className="space-y-1.5">
        <Label htmlFor="naechste_veranstaltungen">Nächste Veranstaltungen</Label>
        <MultiCombobox
          id="naechste_veranstaltungen"
          placeholder="Veranstaltungen auswählen"
          items={veranstaltungenListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.veranstaltungsnummer ?? r.record_id),
          }))}
          values={extractRecordIds(fields.naechste_veranstaltungen)}
          onChange={ids => setFields(f => ({ ...f, naechste_veranstaltungen: ids.length ? ids.map(id => createRecordUrl(APP_IDS.VERANSTALTUNGEN, id)) as any : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateVeranstaltungen("naechste_veranstaltungen", q)}
          createLabel="Neu in Veranstaltungen"
        />
      </div>
    ),
    'gesamtteilnehmerzahl': (
      <div key="gesamtteilnehmerzahl" className="space-y-1.5">
        <Label htmlFor="gesamtteilnehmerzahl">Gesamte Teilnehmerzahl</Label>
        <Input
          id="gesamtteilnehmerzahl"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'gesamtteilnehmerzahl')}
          placeholder="z. B. 156"
          value={fields.gesamtteilnehmerzahl !== undefined ? fields.gesamtteilnehmerzahl : (computedValues['gesamtteilnehmerzahl'] ?? '')}
          onChange={e => setFields(f => ({ ...f, gesamtteilnehmerzahl: clampNumberValue(formEnhancements, 'gesamtteilnehmerzahl', e.target.value) }))}
        />
      </div>
    ),
    'sync_status_gesamt': (
      <div key="sync_status_gesamt" className="space-y-1.5">
        <Label htmlFor="sync_status_gesamt">Sync-Status (gesamt)</Label>
        <Input
          id="sync_status_gesamt"
          placeholder="z. B. erfolgreich, mit Fehlern"
    ...
    'fehlerhafte_syncs': (
      <div key="fehlerhafte_syncs" className="space-y-1.5">
        <Label htmlFor="fehlerhafte_syncs">Fehlerhafte Synchronisationen</Label>
        <Input
          id="fehlerhafte_syncs"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'fehlerhafte_syncs')}
          placeholder="z. B. 2"
          value={fields.fehlerhafte_syncs !== undefined ? fields.fehlerhafte_syncs : (computedValues['fehlerhafte_syncs'] ?? '')}
          onChange={e => setFields(f => ({ ...f, fehlerhafte_syncs: clampNumberValue(formEnhancements, 'fehlerhafte_syncs', e.target.value) }))}
        />
      </div>
    ),
    'letzter_sync': (
      <div key="letzter_sync" className="space-y-1.5">
    ...
    'spz_last_sync': (
      <div key="spz_last_sync" className="space-y-1.5">
        ...
        <DatePicker
          id="spz_last_sync"
          placeholder="Zeitpunkt des letzten techn. Abgleichs"
          mode="datetime"
  ...
  const FIELD_LABELS: Record<string, string> = {"anzahl_anmeldungen": "Anzahl aktuelle Anmeldungen", "naechste_veranstaltungen": "Nächste Veranstaltungen", "gesamtteilnehmerzahl": "Gesamte Teilnehmerzahl", "sync_status_gesamt": "Sync-Status (gesamt)", "fehlerhafte_syncs": "Fehlerhafte Synchronisationen", "letzter_sync": "Letzter Sync-Zeitpunkt", "spz_id": "SPZ-ID", "spz_source": "SPZ-Quelle", "spz_last_sync": "Letzter Sync-Zeitpunkt (technisch)", "spz_sync_status": "Sync-Status (technisch)", "spz_deleted": "In WEBER.SPZ gelöscht"};
  const CURRENCY_KEYS = new Set<string>(["gesamtteilnehmerzahl"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"naechste_veranstaltungen": {"veranstaltungsnummer": "Veranstaltungsnummer", "veranstaltungstitel": "Veranstaltungstitel", "stufe": "Stufe", "verfahren": "Verfahren", "art": "Art", "startdatum": "Startdatum", "enddatum": "Enddatum", "ort": "Ort", "status": "Status", "max_teilnehmerzahl": "Maximale Teilnehmerzahl", "aktuelle_teilnehmerzahl": "Aktuelle Teilnehmerzahl", "bemerkung": "Bemerkung", "spz_id": "SPZ-ID", "spz_source": "SPZ-Quelle", "spz_last_sync": "Letzter Sync-Zeitpunkt", "spz_sync_status": "Sync-Status", "spz_deleted": "In WEBER.SPZ gelöscht"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.WEBER.SPZ_UEBERSICHT} recordId={recordId} />
              </div>
            )}
          </div>

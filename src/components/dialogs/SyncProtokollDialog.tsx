...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/SyncProtokoll';
...
interface SyncProtokollDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: SyncProtokoll['fields']) => Promise<void>;
  defaultValues?: SyncProtokoll['fields'];
...
export function SyncProtokollDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: SyncProtokollDialogProps) {
  const [fields, setFields] = useState<Partial<SyncProtokoll['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<SyncProtokoll['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'sync_protokoll');
      await onSubmit(clean as SyncProtokoll['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "sync_zeitpunkt": string | null, // YYYY-MM-DDTHH:MM\n  "datentyp": LookupValue | null, // Datentyp (select one key: "anmeldung" | "teilnehmer" | "veranstaltung" | "leistung" | "pruefung") mapping: anmeldung=Anmeldung, teilnehmer=Teilnehmer, veranstaltung=Veranstaltung, leistung=Leistung, pruefung=Prüfung\n  "spz_datensatz_id": string | null, // WEBER.SPZ ID\n  "livingapps_datensatz": string | null, // Datensatz-Referenz\n  "aktion": LookupValue | null, // Aktion (select one key: "erstellt" | "aktualisiert" | "uebersprungen" | "fehler") mapping: erstellt=erstellt, aktualisiert=aktualisiert, uebersprungen=übersprungen, fehler=Fehler\n  "sync_status": LookupValue | null, // Status (select one key: "ok" | "warnung" | "fehler") mapping: ok=ok, warnung=Warnung, fehler=Fehler\n  "fehlermeldung": string | null, // Fehlermeldung\n  "rohdaten": string | null, // Rohdaten / Technische Details\n  "spz_id": string | null, // SPZ-ID\n  "spz_source": string | null, // SPZ-Quelle\n  "spz_last_sync": string | null, // YYYY-MM-DDTHH:MM\n  "spz_sync_status": string | null, // Sync-Status\n  "spz_deleted": boolean | null, // In WEBER.SPZ gelöscht\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<SyncProtokoll['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Sync-Protokoll bearbeiten' : 'Sync-Protokoll hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'sync_zeitpunkt': (
      <div key="sync_zeitpunkt" className="space-y-1.5">
        <Label htmlFor="sync_zeitpunkt">Sync-Zeitpunkt</Label>
        <DatePicker
          id="sync_zeitpunkt"
          placeholder="Wann war der Sync?"
          mode="datetime"
    ...
    'datentyp': (
      <div key="datentyp" className="space-y-1.5">
    ...
    'spz_datensatz_id': (
      <div key="spz_datensatz_id" className="space-y-1.5">
        <Label htmlFor="spz_datensatz_id">WEBER.SPZ ID</Label>
        <Input
          id="spz_datensatz_id"
          placeholder="z. B. SPZ-123456"
    ...
    'aktion': (
      <div key="aktion" className="space-y-1.5">
    ...
    'sync_status': (
      <div key="sync_status" className="space-y-1.5">
    ...
    'fehlermeldung': (
      <div key="fehlermeldung" className="space-y-1.5">
        <Label htmlFor="fehlermeldung">Fehlermeldung</Label>
        <Textarea
          id="fehlermeldung"
          placeholder="Fehlerbeschreibung bei Problemen..."
    ...
    'rohdaten': (
      <div key="rohdaten" className="space-y-1.5">
        <Label htmlFor="rohdaten">Rohdaten / Technische Details</Label>
        <Textarea
          id="rohdaten"
          placeholder="Technische Details, JSON..."
  ...
  const FIELD_LABELS: Record<string, string> = {"sync_zeitpunkt": "Sync-Zeitpunkt", "datentyp": "Datentyp", "spz_datensatz_id": "WEBER.SPZ ID", "livingapps_datensatz": "Datensatz-Referenz", "aktion": "Aktion", "sync_status": "Status", "fehlermeldung": "Fehlermeldung", "rohdaten": "Rohdaten / Technische Details", "spz_id": "SPZ-ID", "spz_source": "SPZ-Quelle", "spz_last_sync": "Letzter Sync-Zeitpunkt", "spz_sync_status": "Sync-Status", "spz_deleted": "In WEBER.SPZ gelöscht"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.SYNC_PROTOKOLL} recordId={recordId} />
              </div>
            )}
          </div>

...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Pruefungen';
...
interface PruefungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Pruefungen['fields']) => Promise<void>;
  defaultValues?: Pruefungen['fields'];
...
export function PruefungenDialog({ open, onClose, onSubmit, defaultValues, recordId, anmeldungenList, teilnehmerList, veranstaltungenList, enablePhotoScan = true, enablePhotoLocation = true }: PruefungenDialogProps) {
  const [fields, setFields] = useState<Partial<Pruefungen['fields']>>({});
  ...
  }), [anmeldungenList, teilnehmerList, veranstaltungenList, ]);
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Pruefungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'pruefungen');
      await onSubmit(clean as Pruefungen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["anmeldung", "teilnehmer", "veranstaltung"]);
        ...
        return merged as Partial<Pruefungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Prüfungen bearbeiten' : 'Prüfungen hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'pruefungsart': (
      <div key="pruefungsart" className="space-y-1.5">
        <Label htmlFor="pruefungsart">Prüfungsart</Label>
        <Select
          value={lookupKey(fields.pruefungsart) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, pruefungsart: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="pruefungsart"><SelectValue placeholder="z. B. Erstprüfung, Wiederholung" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
            <SelectItem value="erstpruefung">Erstprüfung</SelectItem>
            <SelectItem value="wiederholung_1">1. Wiederholungsprüfung</SelectItem>
            <SelectItem value="wiederholung_2">2. Wiederholungsprüfung</SelectItem>
            <SelectItem value="erneuerungspruefung">Erneuerungsprüfung</SelectItem>
            <SelectItem value="rezertifizierungspruefung">Rezertifizierungsprüfung</SelectItem>
            <SelectItem value="ergaenzungspruefung">Ergänzungsprüfung</SelectItem>
          </SelectContent>
        </Select>
      </div>
    ),
    ...
    'ergebnis': (
      <div key="ergebnis" className="space-y-1.5">
        ...
        <Input
          id="ergebnis"
          placeholder="z. B. bestanden, nicht bestanden"
    ...
    'bestanden': (
      <div key="bestanden" className="space-y-1.5">
        <Label htmlFor="bestanden">Bestanden</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="bestanden"
            checked={!!fields.bestanden}
            onCheckedChange={(v) => setFields(f => ({ ...f, bestanden: !!v }))}
          />
          <Label htmlFor="bestanden" className="font-normal">Bestanden</Label>
        </div>
      </div>
    ),
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkung"
          placeholder="Besonderheiten, Noten..."
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.PRUEFUNGEN} recordId={recordId} />
              </div>
            )}
          </div>

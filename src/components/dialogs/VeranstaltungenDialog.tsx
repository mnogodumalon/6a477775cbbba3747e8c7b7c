...
export function VeranstaltungenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: VeranstaltungenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "veranstaltungsnummer": string | null, // Veranstaltungsnummer\n  "veranstaltungstitel": string | null, // Veranstaltungstitel\n  "stufe": LookupValue | null, // Stufe (select one key: "tt1" | "tt2" | "tt3") mapping: tt1=TT1, tt2=TT2, tt3=TT3\n  "verfahren": LookupValue | null, // Verfahren (select one key: "tt2_pb" | "tt2_pe" | "tt2_pi" | "vds") mapping: tt2_pb=TT2-PB, tt2_pe=TT2-PE, tt2_pi=TT2-PI, vds=VdS\n  "art": LookupValue | null, // Art (select one key: "schulung" | "pruefung" | "ergaenzungsschulung" | "ergaenzungspruefung" | "wiederholungspruefung" | "erneuerungspruefung" | "rezertifizierungspruefung") mapping: schulung=Schulung, pruefung=Prüfung, ergaenzungsschulung=Ergänzungsschulung, ergaenzungspruefung=Ergänzungsprüfung, wiederholungspruefung=Wiederholungsprüfung, erneuerungspruefung=Erneuerungsprüfung, rezertifizierungspruefung=Rezertifizierungsprüfung\n  "startdatum": string | null, // YYYY-MM-DDTHH:MM\n  "enddatum": string | null, // YYYY-MM-DDTHH:MM\n  "ort": string | null, // Ort\n  "status": string | null, // Status\n  "max_teilnehmerzahl": number | null, // Maximale Teilnehmerzahl\n  "aktuelle_teilnehmerzahl": number | null, // Aktuelle Teilnehmerzahl\n  "bemerkung": string | null, // Bemerkung\n  "spz_id": string | null, // SPZ-ID\n  "spz_source": string | null, // SPZ-Quelle\n  "spz_last_sync": string | null, // YYYY-MM-DDTHH:MM\n  "spz_sync_status": string | null, // Sync-Status\n  "spz_deleted": boolean | null, // In WEBER.SPZ gelöscht\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'veranstaltungsnummer': (
      <div key="veranstaltungsnummer" className="space-y-1.5">
        <Label htmlFor="veranstaltungsnummer">Veranstaltungsnummer</Label>
        <Input
          id="veranstaltungsnummer"
          placeholder="z. B. VE-2026-001"
          value={fields.veranstaltungsnummer ?? ''}
          onChange={e => setFields(f => ({ ...f, veranstaltungsnummer: e.target.value }))}
        />
      </div>
    ),
    'veranstaltungstitel': (
      <div key="veranstaltungstitel" className="space-y-1.5">
        <Label htmlFor="veranstaltungstitel">Veranstaltungstitel</Label>
        <Input
          id="veranstaltungstitel"
          placeholder="z. B. Brandschutzschulung TT2"
          value={fields.veranstaltungstitel ?? ''}
          onChange={e => setFields(f => ({ ...f, veranstaltungstitel: e.target.value }))}
        />
      </div>
    ),
    'stufe': (
    ...
    'art': (
      <div key="art" className="space-y-1.5">
        ...
        <Select
          value={lookupKey(fields.art) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, art: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="art"><SelectValue placeholder="z. B. Schulung, Prüfung" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
            <SelectItem value="schulung">Schulung</SelectItem>
            <SelectItem value="pruefung">Prüfung</SelectItem>
            <SelectItem value="ergaenzungsschulung">Ergänzungsschulung</SelectItem>
            <SelectItem value="ergaenzungspruefung">Ergänzungsprüfung</SelectItem>
            <SelectItem value="wiederholungspruefung">Wiederholungsprüfung</SelectItem>
    ...
    'ort': (
      <div key="ort" className="space-y-1.5">
        ...
        <Input
          id="ort"
          placeholder="z. B. München, Schulungsraum"
          value={fields.ort ?? ''}
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <Input
          id="status"
          placeholder="z. B. geplant, in Durchführung"
          value={fields.status ?? ''}
    ...
    'max_teilnehmerzahl': (
      <div key="max_teilnehmerzahl" className="space-y-1.5">
        <Label htmlFor="max_teilnehmerzahl">Maximale Teilnehmerzahl</Label>
        <Input
          id="max_teilnehmerzahl"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'max_teilnehmerzahl')}
          placeholder="z. B. 15"
          value={fields.max_teilnehmerzahl !== undefined ? fields.max_teilnehmerzahl : (computedValues['max_teilnehmerzahl'] ?? '')}
          onChange={e => setFields(f => ({ ...f, max_teilnehmerzahl: clampNumberValue(formEnhancements, 'max_teilnehmerzahl', e.target.value) }))}
        />
      </div>
    ),
    'aktuelle_teilnehmerzahl': (
      <div key="aktuelle_teilnehmerzahl" className="space-y-1.5">
        <Label htmlFor="aktuelle_teilnehmerzahl">Aktuelle Teilnehmerzahl</Label>
        <Input
          id="aktuelle_teilnehmerzahl"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'aktuelle_teilnehmerzahl')}
          placeholder="z. B. 8"
          value={fields.aktuelle_teilnehmerzahl !== undefined ? fields.aktuelle_teilnehmerzahl : (computedValues['aktuelle_teilnehmerzahl'] ?? '')}
          onChange={e => setFields(f => ({ ...f, aktuelle_teilnehmerzahl: clampNumberValue(formEnhancements, 'aktuelle_teilnehmerzahl', e.target.value) }))}
        />
      </div>
    ),
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkung"
          placeholder="Sonderwünsche, Besonderheiten..."

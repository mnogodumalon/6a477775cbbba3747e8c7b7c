...
import type { Veranstaltungen, Anmeldungen, Teilnehmer, GebuchteLeistungen, Pruefungen, WeberSpzUebersicht, SyncProtokoll, CreateVeranstaltungen, CreateAnmeldungen, CreateTeilnehmer, CreateGebuchteLeistungen, CreatePruefungen, CreateWeberSpzUebersicht, CreateSyncProtokoll } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- GEBUCHTE_LEISTUNGEN ---
  static async getGebuchteLeistungen(): Promise<GebuchteLeistungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GEBUCHTE_LEISTUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as GebuchteLeistungen[];
    return enrichLookupFields(records, 'gebuchte_leistungen');
  }
  static async getGebuchteLeistungenEntry(id: string): Promise<GebuchteLeistungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GEBUCHTE_LEISTUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as GebuchteLeistungen;
    return enrichLookupFields([record], 'gebuchte_leistungen')[0];
  }
  static async createGebuchteLeistungenEntry(fields: CreateGebuchteLeistungen) {
    return callApi('POST', `/apps/${APP_IDS.GEBUCHTE_LEISTUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'gebuchte_leistungen') });
  }
  static async updateGebuchteLeistungenEntry(id: string, fields: Partial<CreateGebuchteLeistungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.GEBUCHTE_LEISTUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'gebuchte_leistungen') });
  }
  static async deleteGebuchteLeistungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GEBUCHTE_LEISTUNGEN}/records/${id}`);
  }
  ...
  static async getWeberSpzUebersicht(): Promise<WeberSpzUebersicht[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WEBER.SPZ_UEBERSICHT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as WeberSpzUebersicht[];
    return enrichLookupFields(records, 'weber.spz_uebersicht');
  }
  static async getWeberSpzUebersichtEntry(id: string): Promise<WeberSpzUebersicht | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WEBER.SPZ_UEBERSICHT}/records/${id}`);
    const record = { record_id: data.id, ...data } as WeberSpzUebersicht;
    return enrichLookupFields([record], 'weber.spz_uebersicht')[0];
  }
  static async createWeberSpzUebersichtEntry(fields: CreateWeberSpzUebersicht) {
    return callApi('POST', `/apps/${APP_IDS.WEBER.SPZ_UEBERSICHT}/records`, { fields: cleanFieldsForApi(fields as any, 'weber.spz_uebersicht') });
  }
  static async updateWeberSpzUebersichtEntry(id: string, fields: Partial<CreateWeberSpzUebersicht>) {
    return callApi('PATCH', `/apps/${APP_IDS.WEBER.SPZ_UEBERSICHT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'weber.spz_uebersicht') });
  }
  static async deleteWeberSpzUebersichtEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WEBER.SPZ_UEBERSICHT}/records/${id}`);
  }

  // --- SYNC_PROTOKOLL ---
  static async getSyncProtokoll(): Promise<SyncProtokoll[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SYNC_PROTOKOLL}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as SyncProtokoll[];
    return enrichLookupFields(records, 'sync_protokoll');
  }
  static async getSyncProtokollEntry(id: string): Promise<SyncProtokoll | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SYNC_PROTOKOLL}/records/${id}`);
    const record = { record_id: data.id, ...data } as SyncProtokoll;
    return enrichLookupFields([record], 'sync_protokoll')[0];
  }
  static async createSyncProtokollEntry(fields: CreateSyncProtokoll) {
    return callApi('POST', `/apps/${APP_IDS.SYNC_PROTOKOLL}/records`, { fields: cleanFieldsForApi(fields as any, 'sync_protokoll') });
  }
  static async updateSyncProtokollEntry(id: string, fields: Partial<CreateSyncProtokoll>) {
    return callApi('PATCH', `/apps/${APP_IDS.SYNC_PROTOKOLL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'sync_protokoll') });
  }
  static async deleteSyncProtokollEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SYNC_PROTOKOLL}/records/${id}`);
  }

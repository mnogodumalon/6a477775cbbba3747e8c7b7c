...
### Step 5: Build
First run `node scripts/check-lookup-keys.mjs` — it verifies every lookup-key literal against the schema and must exit green; on ERROR, fix the flagged file (keys come from `LOOKUP_OPTIONS`, never invent one) and re-run.
Then run `npm run build`. Fix any TypeScript errors (Read failing file, Edit to fix, rebuild).
...
**Lookup keys come from `LOOKUP_OPTIONS` — NEVER invent one.** A key that merely sounds right 400s at runtime ("must be a valid lookup key") and breaks the whole flow for the end user. Before writing ANY lookup-key literal (in writes like `create…Entry`/`update…Entry` too), read the field's options. `node scripts/check-lookup-keys.mjs` flags every unknown literal.
```tsx
...
// ✅ RIGHT — the literal is copied from LOOKUP_OPTIONS['beitraege']['zahlungsstatus']
